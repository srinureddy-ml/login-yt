...
//         let results=await db.query( "UPDATE product5 SET `Product_Name`=?, `Product_cost`=?,`Person_age`=?,`Order_date`=? where id=?",
//         [data.Product_Name, data.Product_cost,data.Person_age,data.Order_date,id],(err,rows,data)=>{
           
//             if(err)throw err 
//             else{
//                 res.json({
//                     message:"Successfully created products with id",
//                     status:"success",
...
//         var db=req.con;
//         var id=req.params.id;
//         let results =await db.query('select * from product5 where id=?',id,function(err,row){
//             if(err) throw err 
//             else{
//                 res.send({
//                     message:"Success"
